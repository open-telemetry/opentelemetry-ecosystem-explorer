...
- `rate_limiting`: Sample based on the rate of spans per second using a token bucket algorithm implemented by [golang.org/x/time/rate](https://pkg.go.dev/golang.org/x/time/rate). This allows for burst traffic up to a configurable capacity while maintaining the average rate over time. The bucket is refilled continuously at the specified rate and has a maximum capacity for burst handling. When the `processor.tailsamplingprocessor.usetracestate` feature gate is enabled, the policy reports the effective sampling threshold on kept traces; see [Tracestate handling](#tracestate-handling).
- `bytes_limiting`: Sample based on the rate of bytes per second using a token bucket algorithm implemented by [golang.org/x/time/rate](https://pkg.go.dev/golang.org/x/time/rate). This allows for burst traffic up to a configurable capacity while maintaining the average rate over time. The bucket is refilled continuously at the specified rate and has a maximum capacity for burst handling. When the `processor.tailsamplingprocessor.usetracestate` feature gate is enabled, the policy reports the effective sampling threshold on kept traces; see [Tracestate handling](#tracestate-handling).
