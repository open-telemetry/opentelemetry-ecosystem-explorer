...
- `max_event_payload_bytes`: Per-event payload cap, in bytes, used to truncate oversized log events before they are sent to CloudWatch Logs. As of [2025-04-02](https://aws.amazon.com/about-aws/whats-new/2025/04/amazon-cloudwatch-logs-increases-log-event-size-1-mb/) the CloudWatch Logs PutLogEvents API accepts events up to 1 MiB (1048576 bytes); the previous limit was 256 KiB. To preserve backwards compatibility this exporter still defaults to 256 KiB. Set this field to `1048576` (or any value up to that ceiling) to take advantage of the larger service limit. Allowed range: 41 to 1048576. A value of 0 (or omitted) means "use the package default (256 KiB)".
