...
import { Search, Loader2, ExternalLink, Box, Send, ChevronDown } from "lucide-react";

import { PageContainer } from "@/components/layout/page-container";
import { GlowBadge } from "@/components/ui/glow-badge";
import { DetailCard } from "@/components/ui/detail-card";
import { GitHubIcon } from "@/components/icons/github-icon";
import { usePrometheusVersions, usePrometheusComponents } from "@/hooks/use-prometheus-data";
...
export function PrometheusPage() {
  ...
  const { data: versionData, loading: versionsLoading } = usePrometheusVersions();
  ...
  const { data: components, loading: componentsLoading } = usePrometheusComponents(currentVersion);
      ...
      <div className="space-y-8">
        <header className="space-y-4">
          ...
          <p className="text-muted-foreground max-w-2xl text-lg leading-relaxed">
            Explore the Prometheus ecosystem within OpenTelemetry. Discover exporters, client
            libraries, and integrations that bridge the two projects.
          </p>
        </header>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="sdk">Client Libraries</option>
                  </select>
                  <ChevronDown
              ...
              {filteredComponents.length > 0 ? (
                filteredComponents.map((comp) => (
                  <DetailCard
                    key={comp.id}
                    withHoverEffect
                    className="border-border/50 hover:border-primary/30 flex h-full flex-col transition-colors"
                  >
                    <div className="flex flex-1 flex-col space-y-4">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          <div className="bg-primary/10 text-primary flex h-10 w-10 items-center justify-center rounded-xl transition-transform duration-300">
                            {comp.type === "exporter" ? (
                          ...
                          <div className="space-y-1">
                            <GlowBadge
                              variant={comp.distribution === "official" ? "success" : "info"}
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-2">
                          <GlowBadge variant="muted" className="px-1.5 py-0.5 text-[9px]">
                            {comp.type === "sdk" ? "Client Library" : "Exporter"}
                          </GlowBadge>
                          {comp.language && (
                            <GlowBadge variant="info" className="px-1.5 py-0.5 text-[9px]">
                              {comp.language}
                            </GlowBadge>
                          )}
                          {comp.category && (
                            <GlowBadge variant="warning" className="px-1.5 py-0.5 text-[9px]">
                              {comp.category}
                            </GlowBadge>
                          )}
                        </div>
                      ...
                      <div className="border-border/10 flex items-center gap-3 border-t pt-4">
                        {comp.repository && (
                          <a
                            href={`https://github.com/${comp.repository}`}
                            target="_blank"
