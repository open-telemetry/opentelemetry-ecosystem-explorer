...
class TestWriteMarkdown:
    ...
    def test_write_markdown_drops_the_autogenerated_status_section(self, db_writer, temp_db_dir):
        content = (
            "<!-- status autogenerated section -->\n"
            "# OTLP Receiver\n"
            "| Status | |\n"
            "<!-- end autogenerated section -->\n"
            "Receives data via OTLP.\n"
        )

        assert db_writer.write_markdown("otlpreceiver", "abc123def456", content) is True

        written = (temp_db_dir / "markdown" / "otlpreceiver-abc123def456.md").read_text(encoding="utf-8")
        assert written == "Receives data via OTLP.\n"

    def test_write_markdown_strips_stray_html_comments(self, db_writer, temp_db_dir):
        content = "# OTLP Receiver\n<!-- markdown-link-check-disable -->\nReceives data via OTLP.\n"

        assert db_writer.write_markdown("otlpreceiver", "abc123def456", content) is True

        written = (temp_db_dir / "markdown" / "otlpreceiver-abc123def456.md").read_text(encoding="utf-8")
        assert written == "# OTLP Receiver\nReceives data via OTLP.\n"

    def test_write_markdown_skips_a_readme_that_is_only_the_status_section(self, db_writer, temp_db_dir):
        # Returning False leaves markdown_hash unstamped, so the detail page shows
        # no README tab instead of an empty one.
        content = (
            "<!-- status autogenerated section -->\n"
        ...
        assert db_writer.write_markdown("jaegerencodingextension", "abc123def456", content) is False

        assert not (temp_db_dir / "markdown").exists()
        assert db_writer.files_written == 0
    ...
    def test_write_markdown_rewrites_a_stale_published_copy(self, db_writer, temp_db_dir):
        ...
        upstream = "<!-- status autogenerated section -->\n# OTLP Receiver\n<!-- end autogenerated section -->\nBody.\n"
        markdown_file = temp_db_dir / "markdown" / "otlpreceiver-abc123def456.md"
        markdown_file.parent.mkdir(parents=True, exist_ok=True)
        markdown_file.write_text(upstream, encoding="utf-8")

        result = db_writer.write_markdown("otlpreceiver", "abc123def456", upstream)

        assert result is True
        assert markdown_file.read_text(encoding="utf-8") == "Body.\n"
        assert db_writer.files_written == 1
