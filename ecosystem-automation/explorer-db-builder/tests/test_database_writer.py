...
class TestWriteMarkdown:
    ...
    def test_write_markdown_skips_content_that_sanitizes_to_nothing(self, db_writer, temp_db_dir):
        content = "<!-- status autogenerated section -->\n# Spring Web MVC\n<!-- end autogenerated section -->\n"

        assert db_writer.write_markdown("spring-webmvc-6.0", "abc123def456", content) is False

        assert not (temp_db_dir / "markdown").exists()
        assert db_writer.files_written == 0

    def test_write_markdown_returns_true_on_success(self, db_writer):
        assert db_writer.write_markdown("test-lib", "abc123def456", "# Test README") is True
