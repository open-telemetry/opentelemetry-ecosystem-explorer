...
class DatabaseWriter:
    ...
    def _is_current(self, file_path: Path, content: str) -> bool:
        ...
        markdown_hash is derived from the *upstream* README, so it does not move
        when only our sanitizing changes. Without this content check, previously
        published files would keep their stale (unscrubbed) text until upstream
        happened to edit the README. Returns False if the file can't be read, so
        an unreadable copy is replaced rather than trusted.
        """
        try:
    ...
    def write_markdown(self, library_name: str, markdown_hash: str, content: str) -> None:
        ...
        The autogenerated status section and HTML comments outside code fences
        are removed before writing (see
        :mod:`explorer_db_builder.readme_sanitizer`). ``markdown_hash`` still
        identifies the *upstream* content, so the path is unchanged by
        sanitizing.
