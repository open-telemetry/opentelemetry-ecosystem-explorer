...
// FileFormat is the inventory schema version emitted by the Go watcher. Go
// starts its own lineage at 0.1, independent of the Java watcher's format.
...
// Manager stores fused Library records as a versioned inventory under a flat
// directory layout: inventoryDir/v{version}/instrumentation.yaml. It ports the
// Python BaseInventoryManager + InventoryManager mechanics.
type Manager struct {
	inventoryDir string
}
...
// that are not valid semantic versions are skipped.
...
// CleanupSnapshots removes all snapshot version directories and returns the
// number removed.
...
// Save writes the libraries as a versioned inventory at
// inventoryDir/v{version}/instrumentation.yaml under the current file format.
...
// Load reads the versioned inventory for version. A missing inventory yields an
// empty envelope at the current file format.
