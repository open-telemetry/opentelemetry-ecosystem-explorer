...
- [ ] Re-scope and open PR from `feat/84-pr2-navbar` (branched off `feat/84-pr1-theme-system`, will
      ...
      **dormant** code (no consumer); `App.tsx` and `main.tsx` flag wiring reverted. Tests green,
      typecheck clean.
- [ ] Open PR 2b (`feat/84-pr2b-v1-scaffolding`) — introduce `src/v1/`, write a minimal `<V1App />`
...
| 2b  | v1 scaffolding                                        | Introduce `src/v1/`; move dormant navbar into `src/v1/components/layout/`; add minimal `<V1App />` mounting the navbar + a placeholder home (chrome-only until Phase 2 ships a real one); `src/App.tsx` adds the single boundary read swapping `<V1App />` for `<LegacyApp />`. Canonical paths (no `/v1/*` prefix); per-deploy bundle via existing `netlify.toml` pattern.                      | PR 3 (SubNav sits beneath it) | —                           |
