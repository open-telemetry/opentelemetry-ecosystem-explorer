...
- Branch `feat/84-pr1-theme-system` has been created off `feat/84-layout-mockups`. PR 1 work starts
  there.
...
- [ ] Open PR 1 from `feat/84-pr1-theme-system` — theme system plumbing (see Phase 1 table below for
      full scope). No visible UI changes; verification via DevTools + unit tests.
...
| 1   | Theme system                                          | Extend `themes.ts` to light + dark + auto. No-flash init script in `index.html`. Reconcile `index.css` color drift. Persist to `localStorage["td-color-theme"]`.                                         | PR 2 (toggle lives in NavBar) | 🚧 In progress      |
