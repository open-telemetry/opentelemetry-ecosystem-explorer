...
- [ ] Commit and open PR from `feat/84-pr2-navbar` (branched off `feat/84-pr1-theme-system`, will be
      rebased onto `main` once PR 1 merges) — minimal NavBar (logo + Docs + ThemeToggle), wires
      `V1_REDESIGN` into `App.tsx` for the first time. Tests green, typecheck clean.
- [ ] Start PR 3 (`feat/84-pr3-subnav`) once PR 2 is in review.
...
| 2   | NavBar v1                                             | Replace `Header` behind the flag. opentelemetry.io-style chrome (always-dark). Scope cut on 2026-05-11: single "Docs" link to opentelemetry.io, "OpenTelemetry" brand text + `OtelLogo` placeholder for the upstream SVG, `<ThemeToggle />` slotted in. Search and language switcher deferred.                                                                                                   | PR 3 (SubNav sits beneath it) | 🚧 Implemented — pending PR |
